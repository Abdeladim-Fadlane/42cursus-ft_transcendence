    ...
    buttons_friends.forEach(button => {
        button.addEventListener('click', (e) =>
        {
           ...
           // socket part -----------------------------------------------------------------
            ...
            if (Web_socket == null)
                Web_socket = new WebSocket(`ws://${window.location.hostname}:8003/chat/${room_name}/`);
            
            Web_socket.onopen = () =>{
                console.log(`WebSocket server is running on ws://${window.location.hostname}:8003/${room_name}/`);
